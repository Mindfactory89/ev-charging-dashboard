...
if [[ "${RETENTION}" -gt 0 ]]; then
  mapfile -t backup_dirs < <(find "${BACKUP_ROOT}" -mindepth 1 -maxdepth 1 -type d | sort 2>/dev/null || true)
  if [[ "${#backup_dirs[@]}" -ge "${RETENTION}" ]]; then
    remove_count=$(( ${#backup_dirs[@]} - RETENTION + 1 ))
